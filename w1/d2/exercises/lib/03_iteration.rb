...
def substrings(string)
  ...
  while i < string.length
    subs << string[i] unless subs.include?(string[i])
    j = i+1
    sub = string[i] 
...
def subwords(word, dictionary)
  substrings(word).select do |elem|
    dictionary.include?(elem)
  end
end
...
class Array
  def my_map(&prc)
    mapped = []
    my_each { |el| mapped << prc.call(el) }
    mapped
  end

  def my_select(&prc)
    selected = []
    my_each { |el| selected << el if prc.call(el) == true }
    selected
  end
