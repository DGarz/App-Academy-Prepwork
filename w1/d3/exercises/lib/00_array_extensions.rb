...
class Array
  def sum
    self.each.inject(0) { |sum, num| sum + num}
  end
end
...
class Array
  def my_uniq
    ...
    uniq = []
    while i < self.length
      uniq << self[i] unless uniq.include?(self[i])
      i+=1
    end
  ...
  def two_sum
    ...
    self.each_with_index do |num, idx|
      self.each_with_index do |num2, idx2|
        if (num + num2 == 0) && idx != idx2 && idx < idx2
          indexes << [idx, idx2]
        end
      end
    end
...
class Array
  def median
    ...
    if self.length.odd?
        ...
        new = self.sort
        (new[self.length/2] + new[(self.length - 1) / 2])/2.0
    end
  end
end
...
class Array
  def my_transpose
    size = self.first.length
    transposed = Array.new(size) { Array.new(size) } ## creating a multidimensional array of size so transpose is not nil and I can change the points in it
    (0...size).each do |i|
      (0...size).each do |j|
